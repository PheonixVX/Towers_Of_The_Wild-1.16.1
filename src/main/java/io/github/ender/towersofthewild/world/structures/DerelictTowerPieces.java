...
public class DerelictTowerPieces {
	private static final Identifier DERELICT_TOWER_TOP = new Identifier(TowersOfTheWild.MOD_ID, "derelict_tower_top");
	...
	public static void addPieces(StructureManager structureManager, BlockPos absolutePos, BlockRotation rotation, List<StructurePiece> pieces, Random random, DefaultFeatureConfig config) {
		pieces.add(new DerelictTowerPieces.Piece(structureManager, DERELICT_TOWER_BOTTOM, absolutePos, rotation));
		pieces.add(new DerelictTowerPieces.Piece(structureManager, DERELICT_TOWER_TOP, absolutePos, rotation));
	}
	...
	public static class Piece extends AbstractTowerPiece {
		...
		public Piece (StructureManager structureManager, Identifier structurePart, BlockPos absolutePos, BlockRotation rotation) {
			super(structureManager, structurePart, rotation, RegistryHandler.DERELICT_TOWER_PIECE, CENTER_TOP_OFFSETS);
		...
		public Piece (StructureManager structureManager, CompoundTag tag) {
			super(structureManager, RegistryHandler.DERELICT_TOWER_PIECE, tag, CENTER_TOP_OFFSETS);
		}
